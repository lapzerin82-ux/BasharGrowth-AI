...
export class Session {
  ...
  async restore(c, replace) {
    ...
    for (const r of c.patients || []) {
      ...
      await this._put("p", { id: r.id, name: r.name, sex: r.sex, fileNumber: r.fileNumber, dob: iso(r.dobEpochDay), father: r.fatherHeightCm ?? null, mother: r.motherHeightCm ?? null, mph: r.mphCm ?? null, mphManual: !!r.mphManual, notes: r.notes || "", preferredReference: r.preferredReference || null, createdAt: r.createdAt || now, updatedAt: stamp("p", r) }, true);
      np++;
    }
