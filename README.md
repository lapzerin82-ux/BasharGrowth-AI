...
| Charts | **The original CDC Clinical Growth Charts (Set 2)**, shown unmodified as vector pages: birth–36 months and 2–20 years, boys and girls. The app writes on them like a clinician would: red × at the exact age (latest circled), optional trajectory line, name, record #, parents' stature and the measurement table (date, age, weight, length/stature, BMI). Automatic switch to the 2–20-year sheet at 24 months. Axis calibration is fitted to the printed grid lines (see `tools/build_cdc_sheets.py`). WHO charts are available as alternatives in Settings. Pinch-zoom, pan, +/−, reset, tap a × for date/age/value/percentile. |
